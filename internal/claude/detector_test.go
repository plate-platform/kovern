...
import (
	...
	"github.com/plate-platform/kovern/internal/claude"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

// mockDetector is a test double for claude.Detector.
...
// Ensure mockDetector satisfies the interface — compile-time check.
var _ claude.Detector = (*mockDetector)(nil)

func makePolicySpec(enabled bool, turns int) *v1alpha1.LivelockPolicySpec {
	...
	return &v1alpha1.LivelockPolicySpec{
		...
		Remediation: v1alpha1.RemediationSpec{
			...
			EmitEvent:  true,
			FaultConfig: &v1alpha1.FaultConfig{HTTPStatusCode: 429, Duration: "60s"},
		},
	}
}
...
func TestMockDetector_DisabledSemantic_NotCalled(t *testing.T) {
	called := false
	d := &mockDetector{}
	_ = d // We test via the NoOpDetector for the disabled path

	// Use a real AnthropicDetector is not feasible in unit tests (needs API key),
	// so verify the disabled path through a no-op stub instead.
	noOp := claude.NewNoOpDetector()
	spec := makePolicySpec(false, 3) // semantic disabled
...
func TestMockDetector_TooFewTurns(t *testing.T) {
	noOp := claude.NewNoOpDetector()
	spec := makePolicySpec(true, 3)

	// Only 1 turn — not enough to detect livelock
